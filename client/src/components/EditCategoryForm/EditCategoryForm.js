...
import { editProductCategory } from "../../actions/Category.action";

const EditCategoryForm = ({ editProductCategory, data }) => {
  //STATES
  ...
  const onSubmitHandeler = async (values) => {
    setIsLoading(true);
    let check = await editProductCategory(
      values,
      data._id,
      selectedFile,
      selectedFile2
    ...
    <div className={styles.wrapper}>
      <Card bg="white" text="dark" className={`${styles.crd} shadow`}>
        <Card.Header className="d-flex justify-content-center align-items-center">
          <span className={styles.heading}>Edit Category</span>
        </Card.Header>
        <Card.Body>
          <Formik
...
export default connect(null, { editProductCategory })(EditCategoryForm);
