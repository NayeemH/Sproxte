  ...
  try {
    ...
    if (userType === "admin" || userType === "coach") {
      ...
      if (products.length) {
        ...
        // Filter teams
        const teamOrders = order.orders.filter(
          (order) => order.type === "team"
        );
        ...
        const products = await Promise.all([
          ...teamOrders.map((order, i) =>
            new Product({
              ...
              layoutImage:
                order.layoutId &&
                teams[i].layouts.filter(
                  ({ _id }) => _id.toString() === order.layoutId.toString()
                )[0].image,
        ...
        await Promise.all(
          products.map(async (product) => {
            ...
            const buffers = await Promise.all([
              fs.readFile(
                path.resolve(`data/image/small/${product.colorImage}`)
              ),
              fs.readFile(
                path.resolve(`data/image/small/${product.frontImages[0]}`)
              ),
            ]);
      ...
      await Project.findOneAndUpdate(
        { _id: id },
        { $push: { gurdianIds: user._id }, $inc: { count: -1 } }
      );
